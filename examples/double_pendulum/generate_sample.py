...
def solve_analytical(initial_state, times):
    ...
    """
    return odeint(pendulum_analytical, initial_state,
    ...
    for i in tqdm(range(num_traj), leave=False):
        while True:
            ...
            idx_pairs = jnp.where(jnp.diff(jnp.hstack(([False], jnp.logical_or(
                fknown1.ravel() <= 0, fknown2.ravel() >= 0), [False]))))[0].reshape(-1, 2)
...
def load_config_yaml(path_config_file, extra_args={}):
    ...
    # A list of seed for multiple evaluation
    seed_number = m_config.get('seed', 1)

    # Initial point randomly generated from these sets in the training/testing
    x0_init_lb_train = m_config.get(
        'x0_init_lb_train', [-3.14/10, -3.14/10, -1.0, -1.0])
    x0_init_ub_train = m_config.get('x0_init_ub_train', [0, 0, 1.0, 1.0])
    x0_init_lb_test = m_config.get(
        'x0_init_lb_test', [-3.14/10, -3.14/10, -1.0, -1.0])
    x0_init_ub_test = m_config.get(
        'x0_init_ub_test', [3.14/4, 3.14/4, 1.0, 1.0])
...
def main_fn(path_config_file, extra_args={}):
    ...
    rng_key, xTest, xTestRolled = gen_samples(
        rng_key, time_step, num_trajectory_test, size_trajectory, n_rollout, x0_init_lb_test, x0_init_ub_test)
    ...
    coloc_set = jax.random.uniform(rng_key, shape=(
        n_coloc, x0_init_lb_test.shape[0]), minval=x0_init_lb_test, maxval=x0_init_ub_test)

...
def print_trajectories(testTraj, n_state, actual_dt, num_traj):
    ...
    for dim in range(n_state):
        ...
        for traj in range(num_traj):
            plt.plot(time_axis, testTraj[traj, :, dim],
...
def plot_avg_accum_error(time_index, ground_truths, sindy_trajectories, gpsindy_trajectories, noise_sd):
    if len(ground_truths) != len(sindy_trajectories) or len(ground_truths) != len(gpsindy_trajectories):
        raise ValueError(
            "The lengths of ground_truths, sindy_trajectories, and gpsindy_trajectories must be the same.")

    if not (len(ground_truths[0]) == len(sindy_trajectories[0]) == len(gpsindy_trajectories[0])):
        raise ValueError(
            "The lengths of every trajectory inside ground_truths, sindy_trajectories, and gpsindy_trajectories must be the same.")
    ...
    for traj_idx in range(len(ground_truths)):
        sindy_squared_error[traj_idx][0] = np.linalg.norm(
            ground_truths[traj_idx][0] - sindy_trajectories[traj_idx][0])
        gpsindy_squared_error[traj_idx][0] = np.linalg.norm(
            ground_truths[traj_idx][0] - gpsindy_trajectories[traj_idx][0])
        for t in range(1, len(ground_truths[0])):
            sindy_squared_error[traj_idx][t] = sindy_squared_error[traj_idx][t-1] + \
                np.linalg.norm(
                    ground_truths[traj_idx][t] - sindy_trajectories[traj_idx][t])
...
def main():
    ...
    data_file_path = input(
        "Enter the path to the data file to train SINDY with (ex. pregenerated/trajectories/gaussian_noise/0.1.pkl): ")
    ...
    try:
        ...
        sindy_results = [sindy.predict(
            traj[0], num_point_in_traj) for traj in testTraj]
        np.save(f'{data_file_path}_{num_traj}_sindy_cache.npy', sindy_results)

    try:
        gp_sindy_results = np.load(
            f'{data_file_path}_{num_traj}_gpsindy_cache.npy')
        print("USING SAVED GPSINDy")
    except:
        print("Generating GPSINDy...")
        manual_hyperparams = bool(int(input(
            "Manually input hyperparameters for Gaussian Process Regression? (1 or 0)")))
        ...
        gp_sindy_results = [gp_sindy.predict(
            traj[0], num_point_in_traj) for traj in testTraj]
        np.save(f'{data_file_path}_{num_traj}_gpsindy_cache.npy',
                gp_sindy_results)
    ...
    plot_avg_accum_error(time_axis, testTraj, sindy_results,
                         gp_sindy_results, noise_sd)
